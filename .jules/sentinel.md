...
## 2024-06-25 - [HIGH] Prevent timing attacks on worker admin token
**Vulnerability:** The `WORKER_ADMIN_TOKEN` verification in the worker lambda (`packages/api/src/worker.ts`) used a standard equality check (`event.authToken !== expectedToken`), making it vulnerable to timing attacks where an attacker could theoretically guess the token character by character based on response times. Also, the absence of runtime type checking on `event.authToken` exposed the application to fatal `TypeError`s if a non-string value was passed.
**Learning:** Comparing sensitive tokens must always use constant-time operations. Furthermore, before passing untrusted payloads to crypto functions (like `crypto.createHash`), runtime type validation must be performed because TypeScript interfaces do not guarantee runtime safety.
**Prevention:** Use `typeof input === 'string'` to validate inputs, and `crypto.timingSafeEqual` to compare hashed values of tokens instead of standard equality operators.
