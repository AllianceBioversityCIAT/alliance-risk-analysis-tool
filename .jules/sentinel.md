...
## 2023-10-24 - Fix Information Disclosure via Job Enumeration
**Vulnerability:** The API endpoint used to retrieve jobs (`findOne`) queried the database via `findUnique` and then performed a secondary, in-memory check to ensure the user owned the job. It threw a `ForbiddenException` (403) for unowned jobs and `NotFoundException` (404) for jobs that did not exist. This differentiation in error codes based purely on object existence (but not ownership) allows an attacker to brute force or enumerate valid job IDs.
**Learning:** Checking permissions *after* fetching an object can inadvertently disclose existence information if the error mechanisms distinguish between "not found" and "found but access denied".
**Prevention:** Enforce ownership constraints at the database query level by using `findFirst` with a compound `where` clause (e.g., `{ id, createdById: userId }`). This ensures that unauthorized requests and requests for non-existent records both yield a `null` response from the database, which can then be consistently mapped to a generic `NotFoundException` (404).
