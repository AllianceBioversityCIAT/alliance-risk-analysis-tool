...
## 2026-07-11 - [HIGH] Missing Record Ownership Validation (IDOR) via findUnique
**Vulnerability:** Multiple services (AssessmentsService, RiskAnalysisService, ReportService, GapDetectionService, JobsService) were fetching records using Prisma's `findUnique` by ID alone, followed by an in-memory check for ownership, or in some cases, bypassing the check entirely in the database layer. This created an Information Disclosure and potential Insecure Direct Object Reference (IDOR) vulnerability, as `findUnique` could leak the existence of other users' records through errors or timing.
**Learning:** Checking ownership *after* retrieving a record from the database is an anti-pattern that can lead to timing attacks and existence leakage. Database queries should inherently enforce access control constraints.
**Prevention:** To prevent IDOR and Information Disclosure, always verify record ownership at the database level by using Prisma's `findFirst` with compound conditions (e.g., `where: { id, userId }` or `where: { id, createdById: userId }`) instead of `findUnique`, and throw a 404 NotFoundException if missing.
