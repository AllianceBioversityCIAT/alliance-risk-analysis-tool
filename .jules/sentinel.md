...
## 2026-06-19 - [HIGH] Prevent IDOR and Resource Enumeration via Database-level Ownership Checks
**Vulnerability:** Ownership of resources (assessments, risk scores, etc.) was previously validated by fetching the resource using `findUnique` and then throwing a `ForbiddenException` if the `userId` did not match. This allowed attackers to enumerate existing resources by differentiating between `NotFoundException` (resource doesn't exist) and `ForbiddenException` (resource exists but doesn't belong to them).
**Learning:** In-memory ownership checks after fetching by ID inherently leak information about the existence of resources. Ownership checks must be pushed down to the database query level to prevent existence leakage.
**Prevention:** To prevent IDOR and Resource Enumeration, endpoints must verify record ownership at the database level. Use Prisma's `findFirst` with compound conditions (e.g., `where: { id, userId }`) instead of `findUnique`, and throw a generic 404 `NotFoundException` if missing. Avoid fetching with `findUnique` followed by an in-memory ownership check that throws a 403 `Forbidden`.
