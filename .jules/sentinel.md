...
## 2025-02-28 - [Mitigate Timing Attack on Worker Admin Token]
**Vulnerability:** Comparing the `WORKER_ADMIN_TOKEN` with the user-provided token using a standard equality operator (`!==`) exposed the application to timing attacks, allowing an attacker to theoretically guess the token byte-by-byte. Additionally, a direct `crypto.timingSafeEqual` comparison without hashing first is susceptible to a fatal TypeError if the inputs are of unequal lengths.
**Learning:** Security tokens must always be compared in constant time, but `crypto.timingSafeEqual` strictly requires arguments of identical length. If raw string lengths differ, Node.js throws an error, which an attacker can use to differentiate matching lengths or cause a Denial of Service.
**Prevention:** To safely use `crypto.timingSafeEqual`, first hash both the expected and provided strings (e.g., with SHA-256) to ensure consistent lengths before making the constant-time comparison.
