...
## 2024-05-23 - Prevent Resource Enumeration via findUnique Ownership Checks
**Vulnerability:** Endpoints fetching resources (assessments, jobs) used Prisma's findUnique followed by an in-memory ownership check throwing 403 Forbidden. This leaked the existence of records belonging to other users.
**Learning:** Returning 403 Forbidden after verifying existence allows attackers to enumerate valid record IDs (Information Disclosure).
**Prevention:** Always verify record ownership at the database level using Prisma's findFirst with compound conditions (e.g., where: { id, userId }) and throw a 404 NotFoundException if missing.
