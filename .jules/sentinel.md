...
## 2026-03-26 - [HIGH] Prevent IDOR and Resource Enumeration via findUnique
**Vulnerability:** Several domain services (Assessments, Gap Detection, Risk Analysis, Report) fetched resources using Prisma's `findUnique` by ID, and *then* performed an in-memory check to throw a `ForbiddenException` if the `userId` didn't match. This leaks the existence of resources to unauthorized users (resource enumeration) and can lead to Insecure Direct Object Reference (IDOR) vulnerabilities.
**Learning:** Returning a 403 Forbidden after a successful database fetch confirms to an attacker that the resource ID exists. To prevent enumeration, unauthorized access attempts must return the same response (404 Not Found) as querying a non-existent resource.
**Prevention:** To prevent IDOR and existence leakage, endpoints must verify record ownership at the database level. Use Prisma's `findFirst` with compound conditions (e.g., `where: { id, userId }`) instead of `findUnique`, and throw a 404 `NotFoundException` if missing. Avoid fetching with `findUnique` followed by an in-memory ownership check that throws a 403 `Forbidden`.
