...
## 2026-03-27 - [HIGH] Fix Information Disclosure via IDOR
**Vulnerability:** The `findOne` method in `AssessmentsService` used `findUnique` followed by a manual ownership check that threw a 403 Forbidden. This leaked resource existence to unauthorized users by differentiating between a 404 (doesn't exist) and 403 (exists but not owned by you).
**Learning:** Fetching a record by ID and then checking ownership in-memory exposes the existence of the record.
**Prevention:** To prevent IDOR and existence leakage (Information Disclosure), endpoints must verify record ownership at the database level. Use Prisma's `findFirst` with compound conditions (e.g., `where: { id, userId }`) instead of `findUnique`, and throw a 404 NotFoundException if missing.
