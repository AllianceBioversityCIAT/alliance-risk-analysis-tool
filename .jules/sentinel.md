...
## 2026-03-12 - Fix Timing Attack Vulnerability in Worker Auth Token Comparison
**Vulnerability:** The `isWorkerAdminAuthorized` function in `worker.ts` compared the expected token with the provided token using simple string comparison (`event.authToken !== expectedToken`), making it vulnerable to timing attacks. This could theoretically allow an attacker to guess the token character by character based on response times.
**Learning:** Simple string comparison should never be used for tokens, secrets, or passwords, even in internal environments. Using `crypto.timingSafeEqual` directly with unequal strings throws a `TypeError`, so hashing is needed first.
**Prevention:** Always use `crypto.timingSafeEqual` along with SHA-256 hashing to ensure the buffers are of equal length and comparisons happen in constant time.
