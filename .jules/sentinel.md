...
## 2026-03-31 - Prevent Resource Enumeration in Assessments Service
**Vulnerability:** The `findOne` method in `AssessmentsService` checked for assessment existence by `id` before checking ownership, throwing a `ForbiddenException` if the user did not own it. This allowed attackers to enumerate existing assessment IDs by distinguishing between 404 (not found) and 403 (forbidden) responses.
**Learning:** Checking resource existence before ownership and returning different status codes can leak information about the database state (Information Disclosure/IDOR).
**Prevention:** Query the resource using a compound condition (e.g., `where: { id, userId }`) with `findFirst` instead of `findUnique` and return a generic 404 `NotFoundException` if the resource is missing or unowned.
