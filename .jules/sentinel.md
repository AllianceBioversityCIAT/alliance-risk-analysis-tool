...
## 2026-03-12 - [HIGH] Prevent Resource Enumeration via IDOR Checks
**Vulnerability:** IDOR (Insecure Direct Object Reference) mitigation checks were implemented using `findUnique` followed by an in-memory `userId !== assessment.userId` check that threw a `403 Forbidden` exception. This implicitely confirms the existence of the requested resource to an attacker (resource enumeration), violating the principle of failing securely.
**Learning:** Returning `403 Forbidden` for existing but unowned resources leaks existence. Database queries should handle ownership checks inherently to return a clean `404 Not Found` if the record doesn't exist or doesn't belong to the user.
**Prevention:** Always verify record ownership at the database level by utilizing `findFirst` with a compound condition (e.g., `where: { id: resourceId, userId: currentUserId }`). Avoid using `findUnique` followed by an application-level existence check unless existence leakage is acceptable by design.
