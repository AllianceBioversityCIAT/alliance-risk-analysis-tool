...
## 2026-03-12 - [HIGH] Missing Record Ownership Validation (IDOR)
**Vulnerability:** Endpoints fetching sensitive records (like assessments) used `findUnique` by ID, then performed an in-memory check to ensure ownership (`assessment.userId !== userId`), throwing a `ForbiddenException` (403) on failure.
**Learning:** This exposes resource existence (Information Disclosure) to attackers, allowing them to enumerate valid record IDs before getting a 403.
**Prevention:** Verify record ownership at the database level by using `findFirst` with a compound condition (e.g., `where: { id, userId }`). If the record is missing or belongs to someone else, it returns `null`. Then, uniformly throw a 404 `NotFoundException` to obscure whether the resource exists or not.
