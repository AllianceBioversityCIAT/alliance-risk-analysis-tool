...
## 2024-05-22 - Prevent IDOR and Resource Enumeration in Prisma Queries
**Vulnerability:** Endpoints fetching related records (like documents, recommendations) used `findUnique` followed by an in-memory `ForbiddenException` if the record didn't belong to the user/assessment. This allowed an attacker to enumerate the existence of other users' records by observing the 403 response instead of a 404.
**Learning:** In-memory ownership checks after fetching records leak existence information.
**Prevention:** Use `findFirst` to combine the primary key lookup with an ownership check (`userId` or `assessmentId`) in the database query. Return a generic 404 `NotFoundException` if no record matches both conditions.
