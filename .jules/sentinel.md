...
## 2026-04-28 - [CRITICAL] Prevent IDOR and Resource Enumeration in Queries
**Vulnerability:** Ownership of resources (like assessments or assessment documents) was verified by first fetching the resource by its primary key (`findUnique`) and then checking the user ID in-memory (throwing a `ForbiddenException`). This pattern is vulnerable to Insecure Direct Object Reference (IDOR) if the in-memory check is forgotten. Additionally, returning a `403 Forbidden` after finding the resource leaks the existence of that resource to an unauthorized user (Resource Enumeration).
**Learning:** Always use compound database queries (`findFirst` with `where: { id, userId }`) to fetch and verify ownership in a single, atomic step. If the resource doesn't belong to the user, the database simply returns null.
**Prevention:** Avoid `findUnique` for resources owned by users. Use `findFirst` and always include the `userId` in the `where` clause. Throw a `404 NotFoundException` if the resource is null, avoiding any leakage of whether the resource actually exists or not.
