...
## 2024-07-01 - Fix Timing Attack Vulnerability in Worker Authentication
**Vulnerability:** The worker logic (`packages/api/src/worker.ts`) compared the highly sensitive `WORKER_ADMIN_TOKEN` secret against a user-provided token using a simple string equality check (`!==`). This is vulnerable to timing attacks, where an attacker could theoretically guess the token by measuring the time it takes for the application to reject invalid guesses.
**Learning:** Standard string equality checks in Node.js short-circuit upon the first mismatching character, creating a timing side-channel. Furthermore, directly passing user input to `crypto.timingSafeEqual` or hash functions without a runtime type check (e.g. `typeof === 'string'`) can lead to fatal `TypeError`s if an object or other type is provided. `crypto.timingSafeEqual` also requires both buffers to be of equal length.
**Prevention:** Always use `crypto.timingSafeEqual` when comparing sensitive tokens, secrets, or passwords. To ensure equal buffer lengths and further protect against timing discrepancies, hash both the expected secret and the provided input (e.g., using SHA-256) before comparing them. Always validate the runtime type of user input (e.g. `typeof input === 'string'`) before passing it to cryptographic functions to prevent fatal exceptions.
