...
## 2026-05-26 - [CRITICAL] Prevent IDOR and Resource Enumeration via findUnique
**Vulnerability:** Ownership validation checks used `findUnique` followed by an in-memory `userId` match that threw a `ForbiddenException` (403). This leaks the existence of records to unauthorized users, as a valid ID would return a 403 while an invalid ID returns a 404, allowing an attacker to enumerate legitimate resource IDs.
**Learning:** Returning a 403 for a resource the user doesn't own tells the attacker the resource exists. Validating ownership in memory after fetching the record by ID introduces an Information Disclosure vulnerability.
**Prevention:** Always verify record ownership at the database level. Use Prisma's `findFirst` with compound conditions (e.g., `where: { id, userId }`) instead of `findUnique`, and throw a generic 404 `NotFoundException` if the query returns null.
