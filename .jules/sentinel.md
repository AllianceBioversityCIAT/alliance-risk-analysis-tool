...
## 2026-03-21 - [HIGH] Prevent IDOR with Prisma findUnique
**Vulnerability:** Endpoints fetching records (e.g., jobs, assessments) by ID used `findUnique` and subsequently checked for ownership in memory, throwing `ForbiddenException`. This can expose the existence of an object to unauthorized users (Information Disclosure).
**Learning:** Checking ownership after fetching the full object is prone to errors, and it can expose existence if not carefully handled. We should avoid `findUnique` followed by a manual check. However, using compound conditions on non-unique fields (like `userId` with `id`) with `findUnique` causes Prisma and TypeScript errors because `findUnique` expects a unique index.
**Prevention:** Use `findFirst` with compound `where` clauses combining the record `id` and the `userId`. This securely scopes the query directly in the database. If no record matches both conditions, return `NotFoundException`. This hides the existence of the resource while effectively enforcing authorization.
