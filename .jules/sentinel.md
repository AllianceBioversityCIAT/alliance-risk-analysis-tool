...
## 2025-10-23 - Prevent Resource Enumeration via IDOR in Assessments
**Vulnerability:** Ownership checks using `findUnique` followed by throwing `ForbiddenException` expose existence leakage (resource enumeration).
**Learning:** Fetching a record solely by `id` and performing the ownership check (`userId`) in-memory leaks the existence of a resource that does not belong to the user if they query another user's valid ID, enabling Information Disclosure.
**Prevention:** Verify record ownership at the database level by using `findFirst` with a compound query (`where: { id, userId }`) and throw a 404 `NotFoundException` if no match is found, masking existence.
