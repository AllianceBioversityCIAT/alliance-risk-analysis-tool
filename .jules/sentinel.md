...
## 2026-07-09 - Fix Timing Attack Vulnerability in Admin Token Comparison
**Vulnerability:** The `WORKER_ADMIN_TOKEN` authentication in `packages/api/src/worker.ts` used a simple string comparison (`event.authToken !== expectedToken`), which is susceptible to timing attacks. This could allow an attacker to infer the expected token length and content character by character by measuring the response time.
**Learning:** Security tokens and passwords should always be compared in constant time. In Node.js, `crypto.timingSafeEqual` is the right tool, but it throws a `TypeError` if the provided buffers are of unequal lengths, or if the user input isn't a string (which can crash the process in a serverless context like Lambda).
**Prevention:** Always compare sensitive secrets using a constant-time algorithm like `crypto.timingSafeEqual`. To handle potential length mismatches and ensure safety, first hash both the expected secret and the provided secret (e.g., using SHA-256) so they are the same length, and add runtime type checks to ensure user input is a string before passing to `crypto` functions.
