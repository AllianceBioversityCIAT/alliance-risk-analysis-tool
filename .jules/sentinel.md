...
## 2026-06-10 - Timing Attack Vulnerability in Token Comparison
**Vulnerability:** The Worker Lambda `isWorkerAdminAuthorized` function used a simple equality check (`!==`) to compare the provided `authToken` against the expected `WORKER_ADMIN_TOKEN` secret. This exposes the comparison to timing attacks, where an attacker could deduce the token byte by byte based on the time the comparison takes to fail.
**Learning:** String equality operators short-circuit, causing early failure when mismatched characters are found. When comparing sensitive secrets, this allows for timing side-channels. Furthermore, `crypto.timingSafeEqual` requires inputs of the same length, meaning raw token buffers would throw exceptions on length mismatches, causing denial of service.
**Prevention:** Always compare sensitive tokens or API keys using `crypto.timingSafeEqual`. To handle potential length variations and prevent `TypeError` exceptions, hash both the expected and provided strings (e.g., using SHA-256) before passing them to `timingSafeEqual`. This ensures the buffers being compared are exactly the same length securely.
