...
## 2026-07-15 - Prevent Information Disclosure via IDOR fixes
**Vulnerability:** Endpoints handling sensitive entities (Assessments, Jobs, Reports, etc.) fetched records using `findUnique` by ID first, and subsequently threw a `ForbiddenException` if the user didn't own the resource. This pattern leaks the existence of resources (Information Disclosure), enabling resource enumeration by unauthorized users.
**Learning:** Returning a `403 Forbidden` after verifying a resource exists exposes that a valid ID was requested, which can lead to further attacks. To fully protect resources, existence should not be confirmed to users who don't have access.
**Prevention:** Always verify record ownership at the database level by using Prisma's `findFirst` with compound conditions (`where: { id, userId }` or similar) instead of `findUnique`, and throw a generic `404 NotFoundException` if the record is missing or inaccessible. Avoid fetching with `findUnique` followed by an in-memory ownership check that throws a `403 Forbidden`.
