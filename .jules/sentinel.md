...
## 2026-02-26 - Prevent IDOR and Resource Enumeration via findFirst
**Vulnerability:** Endpoints fetching records (e.g., Assessments, Jobs) used `findUnique({ where: { id } })` followed by an in-memory ownership check (`if (record.userId !== userId) throw new ForbiddenException(...)`). This approach leaks the existence of the resource (Information Disclosure/Resource Enumeration) because a 403 Forbidden confirms the ID exists, whereas a 404 is returned if it doesn't.
**Learning:** Returning different HTTP status codes (403 vs 404) depending on whether a record exists and belongs to someone else allows an attacker to enumerate valid resource IDs.
**Prevention:** To prevent both IDOR and existence leakage, always verify record ownership at the database query level. Use Prisma's `findFirst` with compound conditions (e.g., `where: { id, userId }` or `where: { id, createdById: userId }`) and throw a 404 `NotFoundException` if no match is found, ensuring consistent responses for unauthorized or missing records.
