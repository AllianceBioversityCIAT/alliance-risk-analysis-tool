...
## 2026-03-07 - [HIGH] Fix timing attack vulnerability in worker auth token
 **Vulnerability:** The authentication check for `WORKER_ADMIN_TOKEN` in `packages/api/src/worker.ts` used a simple string equality comparison (`!==`), making it vulnerable to timing attacks. Furthermore, the type of the `event.authToken` payload was not validated at runtime, opening the door for fatal `TypeError`s if a non-string object was provided.
 **Learning:** Comparing secrets using standard string equality leaks information about the secret via early exits upon the first character mismatch. To mitigate this safely in Node.js, we must use `crypto.timingSafeEqual` with inputs of equal length (e.g., by hashing them first). In addition, TypeScript interfaces don't guarantee runtime types, so we must always validate untrusted input (e.g., `typeof event.authToken === 'string'`) before passing it to strict Node APIs like `crypto.createHash()`.
 **Prevention:** Use `crypto.timingSafeEqual` with SHA-256 hashed buffers for all sensitive token comparisons and perform explicit runtime type checks for any user-provided data.
