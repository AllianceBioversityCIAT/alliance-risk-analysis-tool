...
## 2026-04-25 - Prevent Resource Enumeration (IDOR) via findUnique in Endpoints
**Vulnerability:** Endpoints using `findUnique` followed by a 403 `ForbiddenException` for ownership checks allow attackers to enumerate valid resource IDs.
**Learning:** Checking ownership in-memory after a database fetch leaks the existence of the resource to unauthorized users.
**Prevention:** Use `findFirst` with compound conditions (e.g., `where: { id, userId }`) and throw a 404 `NotFoundException` to mask the existence of resources from unauthorized users.
