...
## 2026-06-05 - [CRITICAL] Prevent IDOR and Resource Enumeration in Service Queries
**Vulnerability:** Backend services (`risk-analysis.service.ts`, `gap-detection.service.ts`, `report.service.ts`, `jobs.service.ts`) used `findUnique` followed by an in-memory `ForbiddenException` to check for resource ownership, which leaked whether specific resources existed to unauthorized users (returning 403 instead of 404).
**Learning:** Checking ownership at the application level rather than via the database query allows attackers to perform resource enumeration. Any endpoint responding with 403 for unauthorized access to a specific ID implicitly validates that the ID exists.
**Prevention:** Roll ownership constraints into the database query (e.g., `findFirst({ where: { id, userId } })`) and return generic `NotFoundException` (404) whenever the query misses to uniformly mask existence from unauthorized users.
