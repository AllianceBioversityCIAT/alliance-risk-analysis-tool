...
## 2026-06-07 - Prevent Timing Attacks in Worker Authentication
**Vulnerability:** The worker handler in `packages/api/src/worker.ts` used standard string comparison (`event.authToken !== expectedToken`) to verify the admin authentication token (`WORKER_ADMIN_TOKEN`). Standard string comparison exits early when it encounters a mismatch, leaking the time taken to evaluate the string, which allows an attacker to guess the correct token character by character via a timing attack.
**Learning:** String equality checks for sensitive values (passwords, tokens, API keys) must always use constant-time comparison algorithms to protect against timing attacks, regardless of the deployment environment (e.g., Lambda functions handling sensitive background jobs). Also, when using `crypto.timingSafeEqual`, the buffers being compared must be of the same length, otherwise a `TypeError` will be thrown. The best practice is to compute a hash of both strings and compare the hashes.
**Prevention:** Always use `crypto.timingSafeEqual` with hashing (like SHA-256) when comparing sensitive tokens or API keys to ensure a constant time comparison and prevent fatal `TypeError`s by ensuring equal buffer lengths.
