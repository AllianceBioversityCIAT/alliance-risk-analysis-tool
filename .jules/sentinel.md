...
## 2026-05-25 - Prevent IDOR and existence leakage with Compound Queries
**Vulnerability:** IDOR (Insecure Direct Object Reference) and existence leakage (Information Disclosure). When checking record ownership, endpoints fetched records using `findUnique` by ID, and if the record was found but did not belong to the user, they threw a 403 Forbidden. This allowed malicious users to enumerate IDs to discover which records existed in the system (since non-existent ones returned 404, while existing but inaccessible ones returned 403).
**Learning:** Checking for ownership in memory after fetching a record via `findUnique` leaks whether the resource exists or not.
**Prevention:** To prevent existence leakage and IDOR, endpoints must verify record ownership at the database level by querying `findFirst` with a compound condition matching both the object ID and the user's ID (e.g., `where: { id, userId }` or `where: { id, createdById: userId }`). If no record is found, the endpoint should always throw a 404 NotFoundException, giving no indication of whether the resource actually exists or if it simply belongs to another user.
