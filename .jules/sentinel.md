...
## 2026-03-22 - [CRITICAL] Prevent IDOR and Resource Enumeration in Resource Access
**Vulnerability:** Endpoints such as `GET /api/assessments/:id` used Prisma's `findUnique({ where: { id } })` followed by an in-memory ownership check (`if (record.userId !== userId) throw new ForbiddenException()`). This allowed attackers to enumerate whether resources existed or not based on whether they received a 403 Forbidden (exists but not owned) or a 404 NotFound (does not exist).
**Learning:** Database queries for user-owned resources must verify ownership at the query level to prevent existence leakage. Relying on application-level checks after fetching exposes information.
**Prevention:** Use Prisma's `findFirst` with compound conditions (`where: { id, userId }`) instead of `findUnique`. If the query returns `null`, always throw a `404 NotFoundException` regardless of whether the resource doesn't exist or isn't owned by the user.
