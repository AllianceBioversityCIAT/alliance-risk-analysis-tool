...
## 📂 Project Directory Boundaries (CGIAR Risk Intelligence Tool)

Use these boundaries when judging task independence for parallel Implementers:

| Boundary | Path | Parallelism rule |
|----------|------|------------------|
| API domain | `packages/api/src/domain/` | Independent per submodule (`assessments`, `gap-detection`, `risk-analysis`, `report`) if files don't overlap |
| API platform | `packages/api/src/platform/` | Independent per module (`auth`, `admin`, `prompts`, `jobs`) |
| API infrastructure | `packages/api/src/infrastructure/` | Serialize if touching shared services (bedrock, storage) |
| API common | `packages/api/src/common/` | Serialize — global guards/filters affect all routes |
| Web frontend | `packages/web/src/` | Independent per feature folder under `components/` or `app/` routes |
| Shared contract | `packages/shared/src/` | **Build first** — serialize before API/Web tasks that import new types |
| Infrastructure | `infra/` | Serialize with API deploy tasks |
| Prisma schema | `packages/api/prisma/` | Serialize — migrations block dependent tasks |

**Monorepo build order:** `shared` → `api` / `web`. Flag cross-package tasks that violate this order.

**CodeGraph:** Not initialized (CLI unavailable). Use Explore subagent or targeted `Grep`/`Glob` for architecture surveys.
