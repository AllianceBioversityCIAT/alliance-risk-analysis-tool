...
## 🧪 Project Test Commands (CGIAR Risk Intelligence Tool)

| Suite | Runner | Command |
|-------|--------|---------|
| API unit | Jest + ts-jest | `pnpm --filter @alliance-risk/api test -- --testPathPattern=<pattern>` |
| API e2e | Jest | `pnpm --filter @alliance-risk/api test:e2e` |
| Web unit | Jest + jsdom + Testing Library | `pnpm --filter @alliance-risk/web test -- --testPathPattern=<pattern>` |
| Infra | Jest snapshot | `pnpm --filter @alliance-risk/infra test` |
| Full monorepo | — | `pnpm test` |

### Test file conventions
- API: `*.spec.ts` colocated with source under `packages/api/src/`
- Web: `__tests__/*.test.tsx` or `*.test.ts` under `packages/web/src/`
- Web script uses `--passWithNoTests` — absence of tests is not automatic PASS; record GAP if scenarios uncovered

### Manual UAT reference
- `docs/testing/analyst-test-protocol.md` — 17-step smoke path for integration scenarios
