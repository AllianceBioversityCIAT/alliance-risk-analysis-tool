...
describe('AuthController (e2e) - Throttling', () => {
  ...
  it('/auth/login (POST) - should throttle after 5 requests', async () => {
    ...
    const payload = { email: 'test@example.com', password: 'test-password' }; // NOSONAR
  ...
  it('/auth/change-password (POST) - should throttle after 5 requests', async () => {
    ...
    const payload = { previousPassword: 'OldPass1!', proposedPassword: 'NewPass1!' };

    // The decorator on changePassword() sets limit to 5
    for (let i = 0; i < 5; i++) {
      await request(app.getHttpServer())
