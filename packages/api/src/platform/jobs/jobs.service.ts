...
export class JobsService {
  ...
  async findOne(id: string, userId: string): Promise<Job> {
    ...
    // Returning NotFoundException instead of ForbiddenException prevents leaking existence
      ...
      if (job.type === JobType.PARSE_DOCUMENT) {
        ...
        if (input.assessmentId) {
          ...
          if (allParsed) {
            ...
            const pending = allDocs.filter((d) => d.status !== 'PARSED' && d.status !== 'FAILED');
            this.logger.log(
              `Not all documents parsed yet for assessment ${input.assessmentId}. ` +
              `Waiting for ${pending.length} more document(s).`,
            );
          }
        }
      }
