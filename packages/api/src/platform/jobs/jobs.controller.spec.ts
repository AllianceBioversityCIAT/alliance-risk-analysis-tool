...
describe('JobsController', () => {
  ...
  describe('findOne()', () => {
    ...
    it('throws NotFoundException for non-existent job or when user does not own the job', async () => {
      ...
      await expect(controller.findOne('non-existent', otherUser)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
