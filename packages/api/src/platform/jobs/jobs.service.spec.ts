...
describe('JobsService', () => {
  ...
  describe('findOne()', () => {
    it('returns the job when ownership matches', async () => {
      ...
      expect(mockPrisma.job.findFirst).toHaveBeenCalledWith({ where: { id: job.id, createdById: 'user-1' } });
    });

    it('throws NotFoundException when job does not exist or user does not own the job', async () => {
