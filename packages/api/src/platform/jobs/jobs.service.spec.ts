  ...
  describe('findOne()', () => {
    it('returns the job when ownership matches', async () => {
      ...
      expect(mockPrisma.job.findFirst).toHaveBeenCalledWith({
        where: { id: job.id, createdById: 'user-1' },
      });
    });
    ...
    it('throws NotFoundException when user does not own the job (prevent resource enumeration)', async () => {
      mockPrisma.job.findFirst.mockResolvedValue(null); // findFirst returns null if ownership mismatch

      await expect(service.findOne('job-1', 'other-user')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
