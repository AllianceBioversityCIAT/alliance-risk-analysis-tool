...
function isWorkerAdminAuthorized(event: WorkerEvent, logger: Logger): boolean {
  ...
  // SECURITY: Use timingSafeEqual combined with hashing to prevent timing attacks.
  // Hashing both values ensures equal buffer lengths, mitigating fatal TypeErrors
  // while preventing timing-based enumeration of the WORKER_ADMIN_TOKEN secret.
