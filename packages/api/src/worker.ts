...
function isWorkerAdminAuthorized(event: WorkerEvent, logger: Logger): boolean {
  ...
  if (typeof expectedToken !== 'string' || typeof event.authToken !== 'string') {
    logger.error(`Unauthorized ${event.action ?? 'worker'} attempt`);
    return false;
  }

  // SECURITY: Use timingSafeEqual with hashing to prevent timing attacks and buffer length mismatch TypeErrors
