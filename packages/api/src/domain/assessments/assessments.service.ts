...
export class AssessmentsService {
  ...
  async requestUploadUrl(
    ...
    userId: string,
  ): Promise<{ presignedUrl: string; fields: Record<string, string>; documentId: string }> {
    await this.findOne(id, userId); // Ownership check
    ...
    const { url: presignedUrl, fields } = await this.storageService.generatePresignedUploadUrl(
      s3Key,
      dto.mimeType,
    );

    return { presignedUrl, fields, documentId: document.id };
  }
