...
describe('AssessmentsService', () => {
  ...
  describe('findOne', () => {
    it('should return assessment when user owns it', async () => {
      ...
      expect(result).toEqual(mockAssessment);
      expect(mockPrisma.assessment.findFirst).toHaveBeenCalledWith({ where: { id: 'assess-1', userId: 'user-1' } });
    });

    it('should throw NotFoundException when not found or user does not own assessment', async () => {
