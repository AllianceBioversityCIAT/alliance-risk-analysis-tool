...
| Real-world LLM classification accuracy of `detectedCountry` on diverse business plans | Not unit-testable; mitigated by the ≥0.7 confidence gate and a real, live Bedrock call already performed during `/akili-execute` T-004 (confirmed correct detection despite a deliberately mismatched injected country) | `requirements.md` §10; `execution.md` T-004 entry |
