...
### T-003 (re-opened, 2nd time) — Backend: DD-CMV-010, manual `country_of_operation` correction takes precedence

- **Status:** PASS (attempt 1)
- **Found:** the user, using the shipped feature normally, corrected the Core-10 "Country of Operation" field in the Gap Detector. The field itself correctly showed the correction after re-analysis, but the mismatch dialog kept citing the original AI-detected country.
- **Root cause:** the Country Detection prompt instruction (DD-CMV-007) tells the model to determine the country from "only what the documents themselves say" — which, by that same logic, also makes it disregard the "USER-PROVIDED CORRECTIONS" block appended during re-analyze. A re-analyze triggered by editing `country_of_operation` therefore re-derives `detectedCountry` from the same unchanged document text, contradicting the correction the Analyst just made.
- **Scope:** in `processUploadMode()`, reuse the `existingFields` query already run for the corrections-text-building step to also extract `country_of_operation`'s `correctedValue`; if it passes a sanity check (non-empty after trim, ≤100 chars, not literally `"unclear"`) it becomes `detectedCountry` directly — bypassing the ≥0.7 confidence gate entirely, since it's user-authored ground truth, not a second AI opinion — otherwise falls back to the model's own response exactly as before. No prompt change, no schema change, no frontend change. Extracted a shared `sanitizeCountryString()` helper so the AI-response path (`normalizeDetectedCountry()`) and the new correction path share the same three checks without duplicating logic.
- **Implementer verification:** suite 21/21 → 25/25 (4 new tests); `api build` clean; full-repo `tsc --noEmit` shows only the 2 pre-existing unrelated errors.
- **Reviewer verdict:** PASS. **Mutation-tested twice:** reverting the precedence line to always use the model's response broke exactly the 1 correction-wins test (all 24 others green — independently corroborating the refactored `normalizeDetectedCountry()` is behavior-identical to before); removing the sanity check broke exactly the 3 sanity-check tests. Independently confirmed the confidence gate never touches the user's correction (it's applied only inside `normalizeDetectedCountry()`, which the correction path skips via `??` short-circuit), the Bedrock-failure catch block is untouched and unreachable from the new code, and DD-CMV-006's single fold-in write is still intact (exactly one `assessment.update()` call). Also independently traced that FR-CMV-006 Scenario 1 (document replacement clears a stale mismatch) is unaffected — replacement runs with `isReAnalyze: false`, which wipes all corrections via `gapField.deleteMany()` before this new logic could ever see them.
  - **ADVISORY (non-blocking, closed via a design.md wording correction):** the Reviewer noted the rule is broader than "the one re-analyze triggered by editing this field" — once `country_of_operation` carries any correction, it stays pinned across *every* subsequent re-analyze (including ones triggered by editing unrelated fields), since corrections persist indefinitely once set. Correct-by-design (the escape hatch is replacing the document), but the design.md prose was imprecise about this — corrected to state the true scope explicitly.

**Requirements/design refs:** `requirements.md` FR-CMV-006 Scenario 3 (new); `design.md` §6.2 point 1b, §12 DD-CMV-010.
...
| T-003 (re-opened, 2nd time) | PASS | 1 (DD-CMV-010 — manual `country_of_operation` correction takes precedence; found by the user's normal use of the shipped feature) | `[BE]` |
