...
No new enum needed — `detectedCountry` is `string | null` (**revised 2026-08-19, DD-CMV-007:** originally typed as `SupportedCountryLabel | null`, restricting persisted values to the 4-country allowlist; widened to a plain `string` because the value can now be *any* country the AI confidently names, not just one of the 4 — see §6.2/§6.3. The literal string `"unclear"` is still never persisted — see §6.2 — only a non-empty country-name string or `null` ever reaches the database), validated at the point it's written (handler: non-empty, length-bounded, confidence-gated), not via a DB-level enum constraint. This keeps the migration additive.
...
1. **(Revised 2026-08-19, DD-CMV-007 — see judgment below)** On a successful Bedrock parse: normalize `parsed.detectedCountry`. It counts as a real detection if, after trimming whitespace, it is a **non-empty string of at most 100 characters** (matching the DB column's `VarChar(100)` bound — a defensive cap against a pathological response, not a country-name allowlist), is not the literal `"unclear"` (case-insensitive), **and** `parsed.detectedCountryConfidence >= 0.7`. **No membership check against `SUPPORTED_COUNTRY_LABELS` is performed here anymore** — a confidently-detected country outside the 4-country allowlist (e.g. `"Malawi"`) is a valid, real value and must be persisted as-is, not discarded. Anything else (missing key, empty/oversized string, wrong type, low confidence, or `"unclear"`) becomes `null` — fail-quiet per NFR-CMV-011. **Log a single `logger.debug()` line whenever this normalization rejects a value** (e.g. "detectedCountry rejected: value=X confidence=Y") — without it, a silently-inert feature (e.g. an Admin's §6.4 edit omitted `detectedCountryConfidence` from the Output Format example, so every response fails the confidence gate) has no diagnostic trail at all.

   **1b. (New 2026-08-19, DD-CMV-010) — a manual `country_of_operation` correction overrides the model's re-detection during re-analyze.** Immediately before the step above, check whether the Analyst has corrected the Core-10 `country_of_operation` field: fetch it from the same `existingFields` query `processUploadMode()` already runs in re-analyze mode to build the "USER-PROVIDED CORRECTIONS" block (handler.ts, step 4b) — don't issue a second query. If `country_of_operation`'s `correctedValue` is present and, after trimming, is non-empty, ≤100 characters, and not literally `"unclear"` (the **same** three checks as 1 above, reused as a small shared helper — but **skip the confidence gate entirely**, since this value has no AI confidence to check and is user-authored ground truth by construction), use it directly as `detectedCountry` and **do not** call the model-response normalization in step 1 at all for this run. This is why: the Country Detection instruction (§6.3) tells the model to look "only at what the documents themselves say," which is exactly what makes it (correctly, by that instruction's own logic) blind to the "USER-PROVIDED CORRECTIONS" block — so on a re-analyze triggered by editing this exact field, the model would otherwise re-report the same country it found before, contradicting the correction the Analyst just made to the very field this whole feature exists to double-check. **Scope, precisely (corrected after Reviewer scrutiny — the rule is broader than "only the triggering edit"):** this precedence rule applies to **every** re-analyze run for the lifetime of the assessment, once `country_of_operation` carries any correction at all — not just the one re-analyze immediately triggered by editing that specific field. `updateFieldsFromAIResponse()` preserves `correctedValue` indefinitely once set, so a later re-analyze triggered by correcting an unrelated Core-10 field will still find and re-apply the standing `country_of_operation` correction. This is correct-by-design, not a defect: the correction is "ground truth" for as long as it stands, exactly like every other Core-10 correction already behaves. The escape hatch is the existing one — replacing the document (`isReAnalyze: false`) wipes all corrections via `gapField.deleteMany()`, so a genuinely new document gets a genuinely fresh detection (FR-CMV-006 Sc1 is unaffected by this rule). It does not change anything about the confidence gate or allowlist logic in step 1, which remains exactly as DD-CMV-007 left it for every other case.

   **Why this changed:** the original design restricted `detectedCountry` to `SUPPORTED_COUNTRY_LABELS` via `isSupportedCountry()`, treating a confidently-detected unsupported country identically to a genuinely unclear one. The user's own manual browser test before archiving caught this directly: a document describing Malawi against a selected country of Nigeria produced *no* dialog, because the log showed `detectedCountry rejected: value="Malawi" confidence=0.95` — correctly implementing the original (too-narrow) design, but not what the feature is actually for. A mismatch against an unsupported country is, if anything, a stronger signal than a mismatch between two supported ones.
...
**Revised 2026-08-19 (DD-CMV-007):** the original instruction constrained `detectedCountry` to exactly one of the 4 supported countries or `"unclear"`. Manual testing before archive showed this backfires — a business plan describing an unsupported country (e.g. Malawi) either got coerced to `"unclear"` or, when the model didn't strictly follow the enum, got discarded by the handler's now-removed allowlist check. The instruction is widened to ask for whatever real country the documents describe, matching the widened BR-CMV-001.
...
(Wrapped in `!!` so the value is a real `boolean`, not `string | boolean | undefined`, before it reaches any prop or JSX. `isSupportedCountry` is no longer imported for this purpose in this file — it isn't needed once `null` is the backend's only "not a real detection" signal.)
...
- **BR-CMV-004 (graceful fallback, added 2026-08-19):** since `assessment.detectedCountry` can now be a country outside the 4-country allowlist (e.g. `"Malawi"`), verify `CountryBadge`'s existing behavior is sufficient — it is, with **zero code change needed**: `country-badge.tsx` calls `getCountryFlag(country as Parameters<typeof getCountryFlag>[0])`, and `getCountryFlag()` already returns `''` for any value outside `SUPPORTED_COUNTRIES`, which the component renders as an empty `<span>` followed by the plain country name. No broken icon, no thrown error, no blank space where the name should be — already exactly the required degradation.
...
**Visual polish (DD-CMV-009, added 2026-08-19 — the user flagged the shipped dialog as visually awkward, not incorrect):** the first shipped version embedded two `CountryBadge` pills mid-sentence inside `DialogDescription`'s prose ("You selected [🇪🇹 Ethiopia] when this assessment was created, but ... describes operations in [Malawi]. Country context shapes ..."). A pill-shaped `Badge` sitting inline in flowing text has taller line-height and its own padding than surrounding prose, which visibly breaks paragraph rhythm and reads as a layout defect, not a stylistic choice — confirmed by a screenshot the user shared. **No requirement changes** — FR-CMV-002 Sc1's actual clauses ("name both, each with its flag" and "state the check does not block analysis") don't mandate a specific sentence structure, only that both pieces of information are present and legible. Restructure to:

1. A small comparison card **above** the prose (not embedded in it) — two labeled columns, "Selected" and "Detected in document", each showing its `CountryBadge` as a normal block element (no inline text-wrapping awkwardness), separated by a small `ArrowRight` icon (`lucide-react`, already a project dependency).
2. `DialogDescription`'s prose is trimmed to not repeat the country names in words (the comparison card above already shows them) — just the explanatory sentence: *"Country context shapes the regulatory, market, and climate risk factors used in the analysis, so this mismatch can affect how accurate the results are."*
3. The non-blocking reassurance paragraph — **kept verbatim, not reworded** (its exact wording is the JD-15 correction above; do not touch it) — moves into a visually distinct callout box (muted/primary-tinted background, small `Info` icon from `lucide-react`) instead of sitting as a second identical-looking paragraph. This is presentation only; the text inside is byte-identical to the paragraph in the previous version.

This changes JSX structure and Tailwind classes only — no new state, no new requirement, no copy change to the one paragraph that's under a specific correction record. Existing tests that check for country names and the "won't block your analysis" phrase within `role="dialog"` continue to pass unmodified, since both are still present, just laid out differently.
...
**Second required cache-invalidation addition (DD-CMV-008, found 2026-08-19 by a real end-to-end user test the moment DD-CMV-007 landed):** the fix above only covers the **re-analyze** flow, because `jobStatus` comes from `useJobPolling()`, which the frontend only ever starts (`startPolling(jobId)`) inside `handleUpdateField`'s debounced re-analyze call. The very first, automatic `GAP_DETECTION` job — server-chained right after `PARSE_DOCUMENT` completes, with **no** frontend `startPolling()` call — never touches `jobStatus` at all. The frontend instead discovers *that* job finished via `useGapFields()`'s own polling (`use-gap-detection.ts`: `refetchInterval` returns `3000` while `data.total === 0`, and `false` once fields arrive). A real test (fresh upload, not a re-analyze) confirmed this exact path never showed the mismatch dialog even though `Assessment.detectedCountry` was correctly persisted — the `assessment` query was simply never told to refresh.

**Fix:** add a second effect (independent of the `jobStatus` one above) that watches `gapData?.total`'s transition from `0`/`undefined` to a positive count, using a ref to detect the transition exactly once:
...
This is the correct, path-agnostic signal for "the async gap-detection work (of either kind) just finished" — it fires once when fields first populate, regardless of whether that happened via the initial automatic run or a re-analyze (in which case it's a harmless, cheap duplicate of the `jobStatus` effect's own invalidation — React Query de-dupes concurrent invalidations of the same key). No new hook, no new query key — same pattern as the first fix.
...
| `detectedCountry` present but empty/oversized/wrong type, or `detectedCountryConfidence < 0.7` | Normalized to `null`, logged at `debug` level — no dialog (fail-quiet, never fail-loud). **Revised 2026-08-19:** being outside the 4-country allowlist is no longer, by itself, a rejection reason — see §6.2/DD-CMV-007 |
...
| Valid supported country / valid **unsupported** country (e.g. "Malawi") — both at confidence ≥0.7 — / `"unclear"` / empty string / missing key / confidence <0.7 → correct normalized `Assessment.update` call in each case (**revised 2026-08-19**: the "unsupported country" case now asserts the value is *persisted*, not rejected — replaces the old "hallucinated string → null" test) | `gap-detection.handler.spec.ts` (extend) | Defect class: wrong persistence/normalization; BR-CMV-001/003 enforcement |
...
| Dialog shown on true mismatch — **both** when the detected country is one of the 4 supported ones and when it isn't (e.g. "Malawi" — **revised 2026-08-19**, previously this case was asserted to skip the dialog); match or `null` still correctly skip it | `.../gap-detector/__tests__/gap-detector-client.test.tsx` | Defect class: dialog logic bug; BR-CMV-001 |
...
| DD-CMV-007 | *(New 2026-08-19, post-`/akili-validate`)* Widen `detectedCountry` to accept **any** confidently-detected country string, not just the 4 `SUPPORTED_COUNTRY_LABELS` — remove the `isSupportedCountry()` gate from both the handler's normalization (§6.2) and the frontend's mismatch derivation (§7); keep only the confidence gate (BR-CMV-003) and a defensive non-empty/length-bounded string check | Keep the 4-country restriction and treat unsupported-country detections the same as `"unclear"` (the original design, and what shipped through `/akili-validate`) | The user's own manual browser test before archiving surfaced this directly: selecting Nigeria against a document describing Malawi produced no dialog at all, logged as `detectedCountry rejected: value="Malawi" confidence=0.95` — the code was correctly implementing the original design, but the original design was wrong. A business plan describing a country the platform doesn't even support is exactly the scenario this feature exists to flag, not a case to silently discard. `CountryBadge`'s existing empty-string flag fallback (verified, zero code change needed) already satisfies the new BR-CMV-004 display requirement this decision introduces |
| DD-CMV-008 | *(New 2026-08-19, found immediately after DD-CMV-007 by the same user re-test)* Invalidate `['assessment', id]` when `gapData.total` transitions from `0` to a positive count (tracked via a `useRef`), **in addition to** the existing `jobStatus === COMPLETED` invalidation — not instead of it | Rely only on the existing `jobStatus === JobStatus.COMPLETED` effect (design.md §7, closed a round-3 Judgment Day SEVERE finding) | That effect is driven by `useJobPolling()`, which the frontend only ever starts (`startPolling(jobId)`) for the **re-analyze** flow (`handleUpdateField`'s debounced call). The very first, automatic `GAP_DETECTION` job — triggered server-side by chaining after `PARSE_DOCUMENT`, with no frontend `startPolling()` call at all — is invisible to that effect; the frontend instead discovers it finished by `useGapFields()`'s own 3-second poll-until-`total>0` loop (`use-gap-detection.ts`). A real end-to-end test (first-time upload, not a re-analyze) showed the dialog never appears on this path, even with a correctly-persisted `detectedCountry` in the database — the client cache for `assessment` was simply never told to refresh. Watching `gapData.total`'s 0→positive transition is the correct, path-agnostic signal for "the async gap-detection work just finished," covering the initial run without disturbing the existing re-analyze-specific effect |
| DD-CMV-009 | *(New 2026-08-19, pure presentation)* Restructure the mismatch dialog's body into a labeled comparison card (Selected / Detected in document, each a block-level `CountryBadge`) above a trimmed `DialogDescription`, with the non-blocking reassurance text moved (verbatim) into a distinct callout box | Leave the original layout, which embedded two `CountryBadge` pills mid-sentence inside `DialogDescription`'s prose | The user flagged the shipped dialog as visually awkward — a pill-shaped `Badge` inline in flowing text breaks paragraph line-rhythm. No requirement changed; FR-CMV-002 Sc1 only requires both countries + flags visible and the non-blocking statement present, not a specific sentence structure |
| DD-CMV-010 | *(New 2026-08-19)* During re-analyze, if the Analyst has manually corrected the Core-10 `country_of_operation` field, that correction — after the same non-empty/length/`"unclear"` sanity check as any `detectedCountry` value, but **without** the ≥0.7 confidence gate — becomes `Assessment.detectedCountry` directly, overriding whatever the model independently re-derives from the (unchanged) document text | Leave `detectedCountry` fully independent of Core-10 field corrections, as it was through DD-CMV-007/008/009 | The user found this by using the product as intended: correcting `country_of_operation`, watching the Core-10 field update correctly, then finding the mismatch dialog still cited the *original* AI-detected country. Root cause: the Country Detection instruction (§6.3, DD-CMV-007's anti-anchoring wording) tells the model to rely "only on what the documents themselves say," which — correctly, by that same instruction's own logic — also causes it to disregard the "USER-PROVIDED CORRECTIONS" block appended for re-analyze runs. The fix does not touch the prompt (which would risk reopening the anchoring question DD-CMV-007 already closed) — it's a deterministic, code-level precedence rule: a user's own correction to the exact field this whole feature is about is ground truth, exactly as the existing "treat as ground truth" convention for Core-10 corrections already establishes, not a second AI opinion needing its own confidence score |
