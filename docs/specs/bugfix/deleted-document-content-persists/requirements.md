...
| OQ-2 | Root `CLAUDE.md` mandates `ApiResponse<T>` for all API responses, but **zero endpoints use it** — all 125 `ApiResponse` occurrences in `packages/api/src` are the unrelated `@nestjs/swagger` decorator. Complying here would make this the only enveloped endpoint | **Routed 2026-08-26 → `KZ-011`** in `docs/specs/kaizen-log.md`. Validation found the escalation honest but **unrouted** — an Open Questions row is archived with its spec, so the question would have died here. It now lives where every AKILI command reads it, with `/akili-constitution` named as owner and the decision stated: amend the rule to match reality, or plan a migration |
