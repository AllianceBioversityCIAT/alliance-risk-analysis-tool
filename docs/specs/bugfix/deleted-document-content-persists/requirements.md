...
| ID | Question | Status |
|----|----------|--------|
| OQ-1 | Exact copy for the withheld-content notice. `docs/ux-ui/design.md:213` records empty-state copy as unfinalised, so no house pattern exists to inherit | **Open** — proposed copy in `design.md` §8.1, confirm at the HITL pause |
...
| OQ-3 | Mounting the document panel for all UPLOAD assessments changes the loading-state layout on the normal path (`design.md` §14) | **Open** — a visible change the Analyst sees on every upload; confirm at the HITL pause |
