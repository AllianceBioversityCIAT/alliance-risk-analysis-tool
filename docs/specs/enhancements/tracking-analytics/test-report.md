...
| File | Tests | Origin |
|---|---|---|
| `src/components/analytics/__tests__/google-analytics.test.tsx` | 3 | 2 authored at T-001 (execute), 1 assertion pair added this pass (merged into the existing "renders with ID" test) |
| `src/components/analytics/__tests__/microsoft-clarity.test.tsx` | 2 | 1 authored at T-002 (execute), 1 assertion added this pass (merged into the existing "injects with ID" test) |
| `src/hooks/__tests__/use-analytics-pageview.test.ts` | 4 | 3 authored at T-001 (execute), 1 new test added this pass |
...
| FR-TRK-003 | Build succeeds without env vars | Build command | `pnpm --filter @alliance-risk/web build` (no env) | PASS (execute-time, `execution.md` T-001/T-003) | Not re-run this pass; no code changed since |
...
| Clarity dashboard masking mode (Strict) was never verified as actually set | It's a manual dashboard setting outside this repo, per design.md §8 / Judgment Day C4/N2 | Must be confirmed in the PR description before this feature ships to production — carried in `execution.md` §3 as an outstanding Leader action, still open at time of this report |
